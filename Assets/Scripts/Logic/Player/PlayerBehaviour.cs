...
public sealed class PlayerBehaviour : SlingEntity
{
    ...
    [SerializeField] private float _comboRushPreEffectDelay = 0.5f; // 줌인 후 이펙트 재생까지 텀
    ...
    [SerializeField] private float _comboRushShake = 0.6f; // 발동 시 카메라 셰이크 강도
    ...
    private float _effectiveAimScale = 1f;
    ...
    private void OnPropelledHit(GameStartButton gameStartButton, Vector2 contactPoint)
    {
    ...
    private IEnumerator HitStopRoutine(float duration)
    {
        ...
        if (!_isComboRushIntro) Time.timeScale = _isAimSlowing ? _effectiveAimScale : 1f;
    }
    ...
    {
        _isAimSlowing = true;
        _effectiveAimScale = SlingBehaviour.Config.aimSlowScale;
        Time.timeScale = _effectiveAimScale;
        ...
        while (elapsed < config.aimSlowFadeDuration)
        {
            ...
            _effectiveAimScale = Mathf.Lerp(from, 1f, t);
            Time.timeScale = _effectiveAimScale;
    ...
    private IEnumerator ComboRushIntroRoutine()
    {
        ...
        CameraController.BeginConstantShake(_comboRushShake);
    ...
    public void CancelAimSlow()
    {
        ...
        if (!_isComboRushIntro) Time.timeScale = _effectiveAimScale;
        _pointerVisualizer?.SetPressMarkerFill(1f);
        PointerInput.DragThresholdScreenRadius = _defaultDragThreshold;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {            
            ...
            if (collision.gameObject.TryGetComponent<GameStartButton>(out var gameStartButton))
            {
                if (IsPropelled) OnPropelledHit(gameStartButton, collision.GetContact(0).point);
            }
